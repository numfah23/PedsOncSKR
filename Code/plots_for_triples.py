...
# pubmed
pm = pd.read_csv("../Data/Semrep/semmeddb_combined_triples.csv")
# clintrials
ct = pd.read_csv("../Data/Semrep/clintrials_combined_triples.csv")
# textbook
tb = pd.read_csv("../Data/Semrep/textbook_combined_triples.csv")

# gold standard pmid clintrials
gs_pmid = pd.read_csv("../Data/Semrep/PubMedGS_Triples.csv")
gs_ct = pd.read_csv("../Data/Semrep/ClinTrialGS_Triples.csv")
gs_tb = pd.read_csv("../Data/Semrep/TextbookGS_Triples.csv")
ideal = pd.read_csv("../Data/Semrep/IdealSemanticRelations_csv.csv")
...
tb_subset = tb[tb["Section"].isin(gs_tb["Section"])]
...
subset_data.to_csv("../Data/Semrep/subset_triples.csv", index=False)

# combine gs into 1 df
all_gs = pd.concat(gs_datas)
all_gs['D_Source'] = all_gs['PMID'].combine_first(all_gs['NCT'])
all_gs['D_Source'] = all_gs['Source'].combine_first(all_gs['Section'])
...
plt.tight_layout()
plt.savefig("plots/ss_preds_barplot")
# plt.show()
...
plt.savefig("plots/gs_preds_barplot")
plt.show()
...
for i in range(len(datas)):
	...
	fig.tight_layout()
	plt.savefig("plots/" + inds[i] + "_preds_with_unq_barplot")
	plt.show()
...
fig.tight_layout()
plt.savefig("plots/all_unique_triples")
plt.show()
...
plt.savefig("plots/venn_diagram_triples")
plt.show()
...
for i in range(len(datas)):
	...
	if i == 2:
		...
		plt.xticks(rotation=90)
		plt.tight_layout()
		plt.savefig("plots/" + inds[i] + "_hist_triple_freq")
		plt.show()
	else:
		if i == 0:
			# group ends
...
for i in range(len(datas)):
	...
	plt.axis("off")
	plt.savefig("plots/" + inds[i] + "_word_cloud")
	plt.show()
...
plt.imshow(wordcloud, interpolation='bilinear')
plt.axis("off")
plt.savefig("plots/integrated_word_cloud")
plt.show()
...
plt.imshow(wordcloud, interpolation='bilinear')
plt.axis("off")
plt.savefig("plots/gs_word_cloud")
plt.show()

# make wordcloud for subset
...
plt.imshow(wordcloud, interpolation='bilinear')
plt.axis("off")
plt.savefig("plots/ss_word_cloud")
plt.show()

#######################################################################################
# make knowledge graphs

# make graph for one nct: NCT01471782 using ss data
...
plt.axis('off')
plt.savefig("plots/ss_graph_NCT01471782")
plt.show()

# make graph for one nct: NCT01471782 using ss data (highlight some edges in red)
...
plt.axis('off')
plt.savefig("plots/gs_graph_NCT01471782")
plt.show()
